...
# Função para buscar os tickers mais negociados da bolsa
...
# Função para obter os percentuais de variação 
def get_stock_changes(data):
    return (data.iloc[0] - data.iloc[0].shift(1)) / data.iloc[0].shift(1) * 100

# Função para extrair informações dos top gainers
...
# Função para plotar o gráfico de setores
...
if df is not None and not df.empty:
    ...
    # Baixar desempenho do último mês das ações
    ...
    # Na primeira coluna, exibe a tabela
    with col1:
        st.write(final_df, height=400) 
